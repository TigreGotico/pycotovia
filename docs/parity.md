...
| `verbos.cpp` — `detectar_desinencias_verbais`, `comprobar_raiz_verbal`, `eliminar_encliticos`, `excep_verbo` | `tra=4` | `morphology.py`, `verb_data.py` | PORTED-PARTIAL: 92.72% exact lemma-set agreement with the binary over the corpus vocabulary; see below |
| `morfolo.cpp` — `analise_morfoloxica` (the non-verb dictionaries) | `tra=4` | — | NOT-PORTED |
...
`morphology.py` ports the verb path of `verbos.cpp`: the reversed ending
dictionary, the root dictionary, the enclitic stripper, and the non-verb
exception list. It answers the question `manexo_do_timbre_verbal()` needs —
which verb, which conjugation model, which tense and person — and it is the
prerequisite for verb timbre.

Scored against the binary's own analysis (`cotovia -SL1lgl`) over the 12638
word types of the parity corpus, of which 3432 have a verb reading on one side
or the other:

| | Count | Share |
|---|---|---|
| Identical lemma set | 3182 | **92.72%** |
| pycotovia offers more | 181 | 5.27% |
| The binary offers more | 67 | 1.95% |
| Disjoint | 2 | 0.06% |

The remainder is `reacentua()`. pycotovia decides whether a graphic accent was
added by an enclitic by re-deriving the stress with its own stress module; the
C reconstructs the accent instead and compares. The two agree almost
everywhere, and where they do not the word is usually one the root dictionary
spells differently (`absterse`, `apóiase`).

The tables in `verb_data.py` are generated from `verbos.txt`,
`desinencias.txt` and the enclitic table in `verbos.cpp` by
`tools/gen_verb_tables.py`. File order is preserved exactly, because both
dictionaries are searched with binary search and the order decides which
entries are reachable.

The gold in `tests/gold/verb_lemmas.tsv` is a frozen extract of the binary's
analysis, regenerated by `tools/gen_verb_gold.py`. It is test gold only. The
analyser never consults it.
...
| Letter case on `e`, `o` | 477 | Vowel timbre, mostly verb forms: `foron`, `temas`, `lemos`, `teñen`. The verb analyser now resolves all four; `manexo_do_timbre_verbal()` itself is the next slice. |
